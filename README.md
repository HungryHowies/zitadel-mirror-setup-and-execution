...
### Zitadel
```
wget https://github.com/zitadel/zitadel/releases/download/v2.59.1/zitadel-linux-amd64.tar.gz
```
```
...
Add the folloing to  new configuration file is for PostgreSQL database. this file will be used for mirror command.
...
Database:
  ...
  postgres:
    ...
    MaxOpenConns: 25 # ZITADEL_DATABASE_POSTGRES_MAXOPENCONNS
    MaxIdleConns: 10 # ZITADEL_DATABASE_POSTGRES_MAXIDLECONNS
    MaxConnLifetime: 1h # ZITADEL_DATABASE_POSTGRES_MAXCONNLIFETIME
...
Steps From the offical documentations
...
```
zitadel init --config /usr/local/bin/defaults-new.yaml
```

### Second commnad executed.

```
zitadel setup --for-mirror --config /usr/local/bin/defaults-new.yaml  --masterkey "MasterkeyNeedsToHave32Characters" --tlsMode external
...
```
 ...
 ### Check PostgreSQl zitadel database before starting zitadel backup.
...
Execute the following command to start Zitadel.
...
### Zitadel Service reconfigure.

Adjust Zitadel's service file by adding the --config flag to the new workign file as shown below.
